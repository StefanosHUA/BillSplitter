...
Uploading an entire receipt's worth of items (often 20+ records) in individual requests would be inefficient and prone to partial failure.

Solution: Designed a Bulk-Add API endpoint that processes a collection of DTOs within a single @Transactional context. By utilizing saveAndFlush and manual persistence context synchronization, the system guarantees that all receipt items are either committed successfully or rolled back entirely, ensuring data consistency.

4. JPA Collection Management and Identity
JPA/Hibernate often deduplicates identical objects in a collection if equality is based on field values rather than unique identifiers.

Solution: Overrode default Lombok equality generation for ReceiptItem entities. By relying on object identity for new instances and primary key identity for persisted instances, the system correctly handles duplicate line items (e.g., multiple identical drinks) without unintended merging.
...
Plaintext
/parea-billSplitter
├── parea-backend/   # Spring Boot REST API and Persistence Layer
├── parea-frontend/  # React SPA and OCR Worker Logic
├── .gitignore       # Root-level VCS exclusion rules
└── README.md
...
Bash
git clone https://github.com/StefanosHUA/BillSplitter.git
cd parea-billSplitter
Backend Setup:
